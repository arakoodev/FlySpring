package com.application.project.myapi.secured;
...
@RestController
@Slf4j
public class Flysecret {
    
        public Mono<ServerResponse> flyget(FlyRequest request) throws Exception{
            try {
                log.info("get secret API");
                return ServerResponse.ok().body(Mono.just("Secret: You Are Awesome"),String.class);
            } catch (Exception e) {
                e.printStackTrace();
